...
void forward_operation(float *x, float *conv1, float *conv2, float *fc1, float *fc2, int *out)
{
    ...
    // Allocate host memory for all of the output maps generated by convolution 1
   	...
   	cudaMalloc((void**)&deviceMask1, xdims[3]*IOLogistics[1]*IOLogistics[2]*sizeof(float));
   	cudaMalloc((void**)&deviceForwardConvInput1, IOLogistics[5]*xdims[3]*xdims[2]*xdims[1]*conv1dims[2]*sizeof(float));
   	cudaMalloc((void**)&deviceForwardConvOutput1, IOLogistics[5]*xdims[3]*outputMapWidth*outputMapHeight*conv1dims[3]*sizeof(float));
   	cudaMalloc((void**)&deviceIOLogistics, IO_LOGISTICS_SIZE*sizeof(int));

    // perform memcopy for the first forward convolution
    cudaMemcpy(deviceMask1, conv1, xdims[3]*IOLogistics[1]*IOLogistics[2]*sizeof(float),cudaMemcpyHostToDevice);
    cudaMemcpy(deviceForwardConvInput1, x, IOLogistics[5]*xdims[3]*xdims[2]*xdims[1]*conv1dims[2]*sizeof(float),cudaMemcpyHostToDevice);
    ...
    std::cout << "POST_MEM_CPY" << std::endl;
